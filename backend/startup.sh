...
if [ -f "ziaroot.crt" ]; then
  keytool -import -trustcacerts -file ziaroot.crt -keystore $JAVA_HOME/lib/security/cacerts -storepass changeit -noprompt
fi

if [ -f "zscaler-ca.crt" ]; then
  keytool -import -trustcacerts -file zscaler-ca.crt -keystore $JAVA_HOME/lib/security/cacerts -storepass changeit -noprompt
fi
