...
public class OpeningServiceIntegrationTest extends AbstractTestContainerIntegrationTest {
    ...
    void getOpeningStockingDetails_existingOpeningId_shouldSucceed() {
        ...
        Assertions.assertEquals("A", dto.stocking().stockingStandardUnit(), "Stocking standard unit should match");
        ...
        Assertions.assertEquals(5.0F, dto.stocking().soilDisturbancePercent(), "Soil disturbance percent should match");

        // Verify preferred species
        Assertions.assertEquals(3, dto.preferredSpecies().size(), "Preferred species size should match");
        Assertions.assertEquals("CW", dto.preferredSpecies().get(0).species().code(), "Preferred species code should match");
        Assertions.assertEquals("western redcedar", dto.preferredSpecies().get(0).species().description(), "Preferred species description should match");
        Assertions.assertEquals(1L, dto.preferredSpecies().get(0).minHeight(), "Preferred species min height should match");

        // Verify acceptable species
        Assertions.assertEquals(1, dto.acceptableSpecies().size(), "Acceptable species size should match");
        Assertions.assertEquals("BA", dto.acceptableSpecies().get(0).species().code(), "Acceptable species code should match");
        Assertions.assertEquals("amabilis fir", dto.acceptableSpecies().get(0).species().description(), "Acceptable species description should match");
        Assertions.assertEquals(1L, dto.acceptableSpecies().get(0).minHeight(), "Acceptable species min height should match");

        // Verify stocking layer
        Assertions.assertNotNull(dto.layer(), "Stocking layer should not be null");
        Assertions.assertEquals(500, dto.layer().minWellspacedTrees(), "Min well-spaced trees should match");
        Assertions.assertEquals(400, dto.layer().minPreferredWellspacedTrees(), "Min preferred well-spaced trees should match");
        Assertions.assertEquals(2, dto.layer().minHorizontalDistanceWellspacedTrees(), "Min horizontal distance well-spaced trees should match");
        Assertions.assertEquals(900, dto.layer().targetWellspacedTrees(), "Target well-spaced trees should match");
        Assertions.assertEquals(800, dto.layer().minPostspacingDensity(), "Min post-spacing density should match");
        Assertions.assertEquals(2000, dto.layer().maxPostspacingDensity(), "Max post-spacing density should match");
        Assertions.assertEquals(10000, dto.layer().maxConiferous(), "Max coniferous should match");
        Assertions.assertEquals(150, dto.layer().heightRelativeToComp(), "Height relative to competition should match");
    }
