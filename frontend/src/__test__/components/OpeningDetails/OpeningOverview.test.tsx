...
describe('OpeningOverview', () => {
  const mockComment: CommentDto = {
    ...
    codeDescriptionDto: { code: 'GENERAL', description: 'General Comments' },
  ...
  test('renders all opening information correctly', () => {
    ...
    expect(screen.getByTestId('card-item-content-management-unit-type')).toHaveTextContent('T - TREE FARM LICENCE');
    expect(screen.getByTestId('card-item-content-management-unit-id')).toHaveTextContent('47');
    expect(screen.getByTestId('card-item-content-timber-sales-office')).toHaveTextContent('LSB - Lumber Sale Branch');
    expect(screen.getByTestId('card-item-content-comment')).toHaveTextContent('Test comment');
  });
