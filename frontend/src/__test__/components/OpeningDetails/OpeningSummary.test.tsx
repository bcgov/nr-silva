...
describe('OpeningSummary', () => {
  ...
  test('renders all opening summary information correctly', () => {
    ...
    expect(screen.getByTestId('card-item-content-opening-number')).toHaveTextContent('92K 014 0.0 514');
    expect(screen.getByTestId('card-item-content-opening-status')).toBeInTheDocument();
    expect(screen.getByText('Free Growing')).toBeInTheDocument();
    
    expect(screen.getByTestId('card-item-content-org-unit')).toHaveTextContent('DAS');
    expect(screen.getByTestId('card-item-content-opening-category')).toHaveTextContent('FTML');
    ...
    expect(screen.getByTestId('card-item-content-client')).toHaveTextContent(expectedClientLabel);
    
    expect(screen.getByTestId('card-item-content-file-id')).toHaveTextContent('TFL47');
    expect(screen.getByTestId('card-item-content-cutting-permit')).toHaveTextContent('12K');
    expect(screen.getByTestId('card-item-content-timber-mark')).toHaveTextContent('47/12K');
    expect(screen.getByTestId('card-item-content-max-allowed-access')).toHaveTextContent('7.8');
    expect(screen.getByTestId('card-item-content-opening-gross-area')).toHaveTextContent('16.6 ha');
    expect(screen.getByTestId('card-item-content-created-by')).toHaveTextContent('BABAYAGA');
    
    // Check formatted dates
    const formattedCreatedOn = formatLocalDate('2001-06-07', true);
    expect(screen.getByTestId('card-item-content-created-on')).toHaveTextContent(formattedCreatedOn || '');
    
    const formattedLastUpdated = formatLocalDate('2014-04-02', true);
    expect(screen.getByTestId('card-item-content-last-updated')).toHaveTextContent(formattedLastUpdated || '');
    
    const formattedDisturbanceStart = formatLocalDate('2001-09-18', true);
    expect(screen.getByTestId('card-item-content-disturbance-start')).toHaveTextContent(formattedDisturbanceStart || '');
